  ...
  try {
    ...
    if (existingFavorite) {
      ...
      await users.findByIdAndUpdate(userId, {
        $pull: { favourite: existingFavorite._id },
      });

      await aama.findByIdAndUpdate(aamaId, { favourite: false });
    ...
    await aama.findByIdAndUpdate(aamaId, { favourite: true });
...
const getFavList = async (req, res) => {
  try {
    ...
    const favDetail = await favourite
      .find({
        by: req.user.id,
      })
      .populate("to");
